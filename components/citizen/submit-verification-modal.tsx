...
export function SubmitVerificationModal() {
  ...
  const verificationTypes = [
    {
      ...
      description: "Submit government-issued ID documents",
      required_docs: ["Government ID", "Selfie"],
    },
    {
      id: "address",
      name: "Address Proof",
      description: "Submit utility bill or lease agreement",
      required_docs: ["Utility Bill", "Bank Statement"],
    },
    {
      id: "tax",
      name: "Tax Certificate",
      description: "Submit tax compliance document",
      required_docs: ["Tax Certificate"],
    },
  ]
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!user?.id) {
        throw new Error("Not authenticated. Please log in to submit verification.")
      }

      console.log("[v0] Submitting verification:", { userId: user.id, type: selectedType })

      const res = await fetch("/api/verifications/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: user.id, type: selectedType }),
      })
      ...
      console.log("[v0] Verification submitted successfully:", data.verification)
      ...
      console.error("[v0] Verification submission error:", error?.message || error)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose the type of verification to submit and provide required documents.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-3">
            {verificationTypes.map((type) => (
              <Card
                ...
                className="p-4 cursor-pointer hover:bg-primary/10 border-border transition-colors"
              >
