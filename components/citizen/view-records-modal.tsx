...
export function ViewRecordsModal() {
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      const transformedRecords = (data.verifications || []).map((item: any) => ({
        id: item.id,
        type: item.documentType,
        title: `${item.documentType.charAt(0).toUpperCase() + item.documentType.slice(1)} Verification Record`,
        status: item.status,
        submittedDate: new Date(item.submittedAt).toLocaleDateString(),
        verifiedDate:
          item.status === "verified" ? new Date(item.reviewedAt || item.submittedAt).toLocaleDateString() : null,
        documentHash: item.documentNumber || "N/A",
      }))
